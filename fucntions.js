...
function findSmallestValue(pixelGrid){
    ...
    for(let y = 0; y<480;y++){
        for(let x=0 ; x<640;x++){
            if(pixelGrid[y][x] > max){max = pixelGrid[y][x]}
            if(pixelGrid[y][x] < min){min = pixelGrid[y][x]}
...
function prewittOperator(pixelGrid) {
  ...
  let pixelGridY = pixelGrid.map(function(arr){return arr.slice();});
  let pixelGridX = pixelGrid.map(function(arr){return arr.slice();});
  ...
  for(let y = 0; y<480;y++){
      for(let x=0 ; x<640;x++){
          pixelGrid[y][x] =  (Math.sqrt(Math.pow(pixelGridY[y][x],2)+Math.pow(pixelGridX[y][x],2)));
